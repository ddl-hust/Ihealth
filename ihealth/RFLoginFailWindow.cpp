...
LRESULT RFDialog::OnNcCalcSize(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL &bHandled) { return 0; }

LRESULT RFDialog::OnNcPaint(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL &bHandled) { return 0; }

LRESULT RFDialog::OnNcHitTest(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL &bHandled) { return HTCLIENT; }

LRESULT RFDialog::OnSize(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL &bHandled) { return 0; }
...
bool RFDialog::OnOKClose(void *pParam)
{
    ...
    if (!actionname.empty()) {
        ...
        if (current_record.target_positions[0].size() > 0 && current_record.target_positions[1].size() > 0 && current_record.target_velocitys[0].size() > 0
            && current_record.target_velocitys[1].size() > 0) {

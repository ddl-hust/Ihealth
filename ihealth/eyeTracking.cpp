...
{
    if (pEyeImage != NULL) delete pEyeImage;
}
...
const cv::Mat EyeTracking::getEyeImage() const { return cv::Mat(*pEyeImage); }

const cv::Point2d EyeTracking::getPupilCenter() const { return this->mPupilCenter; }

const cv::RotatedRect EyeTracking::getPupilEllipse() const { return mPupilEllipse; }
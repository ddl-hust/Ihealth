...
EyeMode::EyeMode(boundaryDetection *_bound)
    : isRunning(false), isCalibrated(false), isSendingData(false), mLeftCamIndex(0), mRightCamIndex(1), bLeftCamOpened(false), bRightCamOpened(false),
      handle_eye(0)
{
    pBound = _bound;
    pEyeControl = NULL;
...
EyeMode::~EyeMode()
{
    ...
    delete pEyeControl;
    if (pLeftEye != NULL) delete pLeftEye;
    if (pRightEye != NULL) delete pRightEye;
}
...
void EyeMode::setVel(double _vel) { pEyeControl->setControlSpeed(_vel); }

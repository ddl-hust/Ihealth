...
int RFMySQLThread::Connect(EventArg *pArg)
{
    ...
    if (pCurrentThread->m_db.Open(cgf.get<std::string>("ip").c_str(), cgf.get<std::string>("user").c_str(), cgf.get<std::string>("password").c_str(), "ihealth",
                                  3306)
        < 0) {
    ...
    if (pLoginInfo != NULL) {
        ...
        if (stmt.Prepare(RFMainWindow::DBThread->m_db, sql) > 0) {
            ...
            if (stmt.Step() > 0) {
                ...
                if (id > 0) {
                    char fields[256] = "id,loginid,hospitalid,employeenumber,name,sex,birthday,certid,telephone,bloodtype,address,education,nation,birthplace,"
                                       "householdprop,department,projgroup,positon,entrydate,flag,createtime";
                    memset(sql, 0, 1024);
...
int RFMySQLThread::FilterPatient(EventArg *pArg)
{
    ...
    std::string fields = "select id,hospitalid,doctorid,name,sex,age,createtime,lasttreattime,totaltreattime,recoverdetail,remarks,flag,saa_rom,sfe_rom from "
                         "patient where flag=0";
...
int RFMySQLThread::Load(EventArg *pArg)
{
    ...
    // char sql[1024] = "";
    char fields[256] =
        "select id,hospitalid,doctorid,name,sex,age,createtime,lasttreattime,totaltreattime,recoverdetail,remarks,flag,saa_rom,sfe_rom from patient where ";
    char condition1[1024];
...
int RFMySQLThread::Save(EventArg *pArg)
{
    ...
    // wsprintf(fields, _T("hospitalid=%d, doctorid=%d, age=%d, flag=%d where id=%d"), pParam->hospitalid, pParam->doctorid, pParam->age, pParam->flag,
    // pParam->id);
...
int RFMySQLThread::Add(EventArg *pArg)
{
    ...
    sql += hospitalid;
    sql += doctorid, sql += _T("'") + pParam->name + _T("',");
...
int RFMySQLThread::AddMultiPatients(EventArg *pArg)
{
    ...
    for (; begin != pParam->patients.end(); begin++) {
        ...
        std::wstring sql =
            _T("INSERT INTO patient (hospitalid,doctorid,name,sex,age,createtime,lasttreattime,totaltreattime,recoverdetail,remarks,flag) value(");
        sql += hospitalid;
        sql += doctorid, sql += _T("'") + begin->name + _T("',");
        sql += _T("'") + begin->sex + _T("',");
        sql += age;
        ...
        if (stmt1.Prepare(RFMainWindow::DBThread->m_db, szSQL) > 0) {
            ...
            if (stmt1.Step() > 0) {
                ...
                sprintf(szsql, "UPDATE patient SET lasttreattime='%s', totaltreattime='%.3f%s', recoverdetail='%s' where id=%d",
                    TGUTF16ToUTF8(pParam->lasttreattime).c_str(), treattime, TGUTF16ToUTF8(_T("小时")).c_str(), TGUTF16ToUTF8(pParam->recoverdetail).c_str(),
                pParam->patientid); RFMainWindow::DBThread->m_db.Exec(szsql);
                */
...
int RFMySQLThread::LoadPatientTrainDetails(EventArg *pArg)
{
    ...
    std::string sql = "select DISTINCT(a.id),a.patientid, a.traindate, a.content, a.duration, a.traintime, a.createtime, a.flag from patienttrain a, patient b "
                      "where a.patientid = b.id and b.flag=0 ";
    char condition[256];
    ...
    if (stmt.Prepare(RFMainWindow::DBThread->m_db, sql.c_str()) > 0) {
        ...
        CTask::Assign(CTask::NotWait, Panic(), pResult, EventHandle(&RFPatientsTrainDetails::OnLoadPatientTrainDetailsOK), pCurrentThread,
                      RFMainWindow::UIThread);
    }
...
int RFMySQLThread::AddPassiveTrainInfo(EventArg *pArg)
{
    ...
    if (pParam->target_pos[0].size() != pParam->target_pos[1].size() && pParam->target_pos[1].size() != pParam->target_vel[0].size()
        && pParam->target_vel[0].size() != pParam->target_vel[1].size() && pParam->target_pos[0].size() > 1) {
        delete pParam;
        pParam = NULL;
        return 1;
    }
